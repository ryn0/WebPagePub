...
using WebPagePub.PageManager.Console.Enums;
...
using WebPagePub.PageManager.Console.WorkFlows.Generators;
...
System.Console.WriteLine("Select which workflow to run from below:");
...
foreach (Workflows workflow in Enum.GetValues(typeof(Workflows)))
{
    ...
    System.Console.WriteLine($"{(int)workflow} - {workflow}");
}

System.Console.Write("Type number and press enter: ");
...
var contentSnippetRepository = serviceProvider.GetService<IContentSnippetRepository>();

if (contentSnippetRepository == null)
{
    throw new NullReferenceException(nameof(contentSnippetRepository));
}
...
switch (workflowSelectionEnum)
{
    ...
    case Workflows.ArticleInternalLinkGenerator:
        ...
        break;
    case Workflows.ArticleImageGenerator:
        var articleImageSettings =
            config.GetRequiredSection("ArticleImageGenerator").Get<ArticleImageGeneratorModel>()
            ?? throw new InvalidOperationException("ArticleImageGenerator settings are missing or not correctly configured.");

        pageEditor = new ArticleImageGenerator(
                chatGptSettings,
