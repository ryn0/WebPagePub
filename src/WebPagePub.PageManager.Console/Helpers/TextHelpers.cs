...
namespace WebPagePub.PageManager.Console.Utilities
{
    public class TextHelpers
    {
        ...
        {
            // Pattern to match the <body> tag and its content until the </body> tag
            string pattern = @"<body.*?>(.*?)<\/body>";

            // Using Singleline to make . match newline characters as well
            Match match = Regex.Match(html, pattern, RegexOptions.Singleline | RegexOptions.IgnoreCase);

            if (match.Success)
            {
                // Return the content within the <body> tags
                return match.Groups[1].Value.Trim();
            }
            else
            {
                // Return an empty string or null, if no <body> tags are found
                return string.Empty;
            }
        }
    }
